...
\begin{longtable}{lcl}
  ...
  \texttt{e} & \(\e\) & base naturelle des logarithmes\\
  \texttt{i} & \(\i\) & base imaginaire des nombres complexes\\
  \texttt{j} & \(\j\) & \(\e^{\i \TAU/3}\) ou quaternion \(j\)\\
  \texttt{k} & \(\k\) & quaternion \(\k\)\\
  \texttt{PI} & \(\PI\) & constante du cercle\\
  \texttt{TAU} & \(\TAU\) & \emph{vraie} constante du cercle\\
  \texttt{gold} & \(\gold\) & nombre d’or\\
  \texttt{eulermasch} & \(\eulermasch\) & constante d’Euler-Mascheroni\\
  ...
  \texttt{biginter} & \(\biginter\) & intersection (grande version)\\
  \texttt{union} & \(\union\) & union\\
  \texttt{bigunion} & \(\bigunion\) & union (grande version)\\
  ...
  \texttt{Algebriques} & \(\Algebriques\) & nombres algébriques\\
  ...
  \multicolumn{3}{l}{\emph{Fonctions usuelles}}\\
  \texttt{ch} & \(\ch\) & cosinus hyperbolique\\
  \texttt{sh} & \(\sh\) & sinus hyperbolique\\
  \texttt{sinc} & \(\sinc\) & sinus cardinal\\
  \midrule
  ...
  \texttt{car} & \(\car\) & caractéristique d’un anneau ou d’un corps\\
  \texttt{Frac} & \(\Frac\) & corps des fractions\\
  \texttt{disc} & \(\disc\) & discriminant\\
  ...
  \texttt{Syl} & \(\Syl\) & sous-groupes de Sylow\\
  \texttt{cyclo} & \(\cyclo\) & polynôme cyclotomique\\
  \multicolumn{3}{l}{\emph{Théorie de Galois}}\\
  \texttt{Gal} & \(\Gal\) & groupe de Galois\\
  ...
  \texttt{Mat} & \(\Mat\) & matrice d’une application linéaire\\
  \texttt{diag} & \(\diag\) & matrice diagonale\\
  ...
  \multicolumn{3}{l}{\emph{Théorie de la mesure}}\\
  \texttt{lebesgue} & \(\lebesgue\) & mesure de Lebesgue\\
  ...
  \multicolumn{3}{l}{\emph{Analyse harmonique}}\\
  \texttt{Torus} & \(\Torus\) & tore\\
  \texttt{fourier\{f\}} & \(\fourier{f}\) & transformée de Fourier de \(f\)\\
  \texttt{Fourier} & \(\Fourier\) & opérateur de Fourier (dans \(\L^1\))\\
  \texttt{Plancherel} & \(\Plancherel\) & opérateur de (Fourier-)Plancherel (dans \(\L^2\))\\
  \midrule
  ...
  \texttt{Cov} & \(\Cov\) & covariance\\
  \texttt{carac} & \(\carac\) & fonction caractéristique\\
  \texttt{gener} & \(\gener\) & fonction génératrice\\
  ...
  \texttt{mobius} & \(\mobius\) & fonction de Möbius\\
  \texttt{legendre\{a\}\{q\}} & \(\legendre{a}{q}\) & symbole de legendre entre \(a\) et \(q\)\\
  \midrule
  \multicolumn{3}{l}{\strong{Géométrie}}\\
  \multicolumn{3}{l}{\emph{Géométrie affine}}\\
  \texttt{Aff} & \(\Aff\) & ensemble d’applications affines\\
  \texttt{GA} & \(\GA\) & groupe affine\\
  \multicolumn{3}{l}{\emph{Géométrie euclidienne}}\\
  \texttt{Is} & \(\Is\) & groupe d’isométries\\
  \bottomrule
\end{longtable}
